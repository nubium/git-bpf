...
class Repository
  ...
  def cmd(*args, ignore_fail: false)
    self.class.git(*(self.ctx + args), ignore_fail: ignore_fail)
  end

  def config(local, *args, ignore_fail: false)
    ...
    cmd(*(self.ctx + command), ignore_fail: ignore_fail)
  end
